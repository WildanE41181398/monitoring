...
float m_kel_kering, m_kel_normal, m_kel_basah;
...
float kelembaban_a1, kelembaban_z1, kelembaban_a2, kelembaban_z2, kelembaban_a3, kelembaban_z3;
float kelembabanAiZi, kelembaban_a, kelembaban_Z;
...
int st_suhu, st_kelembaban;
//--
//int ppm_cukup_min = 1200;
//int ppm_cukup_max = 1400;
//int ppm_rendah = ppm_cukup_min + 50;
//int ppm_tinggi = ppm_cukup_max - 50;
int kelembaban_normal_min = 50;
int kelembaban_normal_max = 60;
int kelembaban_kering = kelembaban_normal_min + 2;
int kelembaban_basah = kelembaban_normal_max - 2;
//--
...
void setup()
{
    ...
    pinMode(SensorKelembabanPin, INPUT);
    //pinMode(ABMIX, OUTPUT); // larutan ab mix
    //pinMode(KOH, OUTPUT); // KOH
    //pinMode(HCL, OUTPUT); // HCl
    pinMode(POMPA, OUTPUT); // hidupkan pompa bak
    pinMode(AIR, OUTPUT); // hidupkan pompa air
}
...
void fuzzy()
{
    ...
    {
        m_suhu_nrmal = (suhu - 23) / (24 - 23);
    }
    else if (28 < suhu && suhu < 29)
    {
        m_suhu_nrmal = (29 - suhu) / (29 - 28);
    }
    else if (24 <= suhu && suhu <= 28)
    {
        m_suhu_nrmal = 1;
    }
    // miu suhu pnas
    ...
    else if (suhu_Z >= 150)
    {
        ...
        Serial.println("suhu terlalu tinggi, hidupkan kipas");
    ...
    // PPM
    // miu kelembaban rndah
    if (kelembaban >= kelembaban_kering)
    {
        m_kel_kering = 0;
    }
    else if (kelembaban_normal_min < kelembaban && kelembaban < kelembaban_kering)
    {
        m_kel_kering = (kelembaban_kering - kelembaban) / (kelembaban_kering - kelembaban_normal_min);
    }
    else if (kelembaban <= kelembaban_normal_min)
    {
        m_kel_kering = 1;
    }
    // // miu kelembaban ckup
    if (kelembaban <= kelembaban_normal_min || kelembaban >= kelembaban_normal_max)
    {
        m_kel_normal = 0;
    }
    else if (kelembaban_normal_min < kelembaban && kelembaban < kelembaban_kering)
    {
        m_kel_normal = (kelembaban - kelembaban_normal_min) / (kelembaban_kering - kelembaban_normal_min);
    }
    else if (kelembaban_basah < kelembaban && kelembaban < kelembaban_normal_max)
    {
        m_kel_normal = (kelembaban_normal_max - kelembaban) / (kelembaban_normal_max - kelembaban_basah);
    }
    else if (kelembaban_kering <= kelembaban && kelembaban <= kelembaban_basah)
    {
        m_kel_normal = 1;
    }
    // // miu kelembaban tnggi
    if (kelembaban <= kelembaban_basah)
    {
        m_kel_basah = 0;
    }
    else if (kelembaban_basah < kelembaban && kelembaban < kelembaban_normal_max)
    {
        m_kel_basah = (kelembaban - kelembaban_basah) / (kelembaban_normal_max - kelembaban_basah);
    }
    else if (kelembaban >= kelembaban_normal_max)
    {
        m_kel_basah = 1;
    }
    ...
    // ppm_a3 = m_ppm_tnggi;
    // ppm_z3 = 50 * 3;
    kelembaban_a1 = m_kel_kering;
    kelembaban_z1 = 50;
    kelembaban_a2 = m_kel_normal;
    kelembaban_z2 = 50 * 2;
    kelembaban_a3 = m_kel_basah;
    kelembaban_z3 = 50 * 3;
    ...
    kelembabanAiZi = (kelembaban_a1 * kelembaban_z1) + (kelembaban_a2 * kelembaban_z2) + (kelembaban_a3 * kelembaban_z3);
    kelembaban_a = kelembaban_a1 + kelembaban_a2 + kelembaban_a3;
    kelembaban_Z = kelembabanAiZi / kelembaban_a;
    ...
    if (kelembaban_Z <= 50)
    {
        Serial.println("kelembaban terlalu kering, tambahkan air");
        digitalWrite(AIR, HIGH);
        delay(5000);
    }
    else if (kelembaban_Z > 50 && kelembaban_Z < 150)
    {
        Serial.println("kelembaban normal");
    }
    else if (kelembaban_Z >= 150)
    {
        Serial.println("kelembaban terlalu basah, air off");
    ...
    */
    ...
    String kirimdata = String(suhu) + "/" + String(kelembaban);
